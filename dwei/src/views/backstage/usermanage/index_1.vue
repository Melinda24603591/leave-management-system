<template>
  ...
  <el-dialog v-model="dialogFormVisible" :title="dialogTitle" width="35%" align-center style="border-radius:10px;" @close="reseForm">
    <el-form :model="form" :rules="userRules" ref="formRef">
      <el-row>
          <el-col :span="12">
            <el-form-item label="name" prop="name" :label-width="80">
              <el-input name="name" v-model="form.name" placeholder="Please fill in your name" autocomplete="off" size="large" :disabled="formState"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="User name" prop="username" :label-width="80">
              <el-input name="username" v-model="form.username"  placeholder="Please fill in your username" autocomplete="off" size="large" :disabled="formState"/>
            </el-form-item>
          </el-col>
      </el-row>
      <el-row>
          <el-col :span="12">
            <el-form-item label="password" prop="password" :label-width="80">
              <el-input name="password" v-model="form.password" placeholder="Please fill in the password" show-password autocomplete="off" size="large" :disabled="formState"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="contact information" prop="phone" :label-width="80">
              <el-input name="phone" v-model="form.phone"  placeholder="Please fill in the phone" maxlength="11" autocomplete="off" size="large" :disabled="formState"/>
            </el-form-item>
          </el-col>
      </el-row>
      ...
      <el-form-item label="email" prop="email" :label-width="80">
...
<script setup lang="ts">
  ...
  import { getUserListApi , addUserApi ,upUserApi,deleteUserApi,resetPwdUserApi} from '@/api/product/myapi'
...
//下拉框列表
  ...
  const getUserListRequset = async(pagenow?:any)=>{
       ...
       const res:any = await getUserListApi(searchForm.value)
  ...
  const deleteUser = async(item:any)=>{
       await deleteUserApi({id:item.id})
       getUserListRequset()
