  ...
  <el-card class="content">
     <!-- <el-button type="primary" :icon="Plus" @click="()=>dialogFormVisible=true">add</el-button> -->
      ...
      <el-table :data="tableData" border max-height="480" :cell-style="{ textAlign: 'center' }" :header-cell-style="{ textAlign: 'center' }" style="width: 100%; margin-top: 10px;">
        ...
        <el-table-column prop="typeName" label="Approval type"/>
        ...
        <el-table-column prop="startDate" label="Start time"/>
        <el-table-column prop="endDate" label="End time"/>
        <el-table-column label="operation" width="250">
          <template #default="scope">
            <el-button type="text" :icon="Edit" title="Approval" @click="updataUser(scope.row)">Approval</el-button>
            <!-- <el-button type="text" :icon="InfoFilled" title="details" style="color:#8e8d8d" @click="lookUserInfo(scope.row)">details</el-button>  -->
            <!-- <el-popconfirm title="您是否确认审批?" @confirm="deleteUser(scope.row)">
                    <template #reference>
                        <el-button type="text" :icon="Edit" title="Approval" style="color:red">Approval</el-button>
                    </template>
            </el-popconfirm>    -->
          </template>
        </el-table-column>
...
<script setup lang="ts">
  ...
  import { getPendingListApi , addTypeListApi ,editTypeListApi,approveApi,resetPwdUserApi,detailTypeListApi} from '@/api/product/myapi'
...
//下拉框列表
  ...
  const getUserListRequset = async(pagenow?:any)=>{
       ...
       searchForm.value.userId = userId
       const res:any = await getPendingListApi(searchForm.value)
